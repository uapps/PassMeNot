...
angular.module('PassMeNot', ['ngRoute', 'ngAnimate', 'ui.bootstrap', 'pass-me-not.templates'])
    ...
    .controller('Controller', ['$scope', '$routeParams', 'Store', '$modal', '$timeout', function ($scope, $routeParams, Store, $modal, $timeout) {

		$scope.identity = angular.identity

        $scope.initialize = function () {
            if ($routeParams.subjects && $routeParams.aims) {
                $timeout(function () {
            ...
            if (Store.has('subjects') && Store.valid('subjects') && !$routeParams.subjects) {
                $scope.subjects = Store.get('subjects')
            } else {
                $scope.subjects = []
            }

            if (Store.has('aims') && Store.valid('aims') && !$routeParams.aims) {
        ...
        $scope.openShareModal = function () {
            $modal.open({
                templateUrl: '/pass-me-not/share.html',
                controller: ['$scope', '$modalInstance', '$window', '$location', function ($scope, $modalInstance, $window, $location) {
                    $location.path('/')
                    $scope.url = $window.location.href
                    $scope.subjects = $window.localStorage.subjects
                    $scope.aims = $window.localStorage.aims
